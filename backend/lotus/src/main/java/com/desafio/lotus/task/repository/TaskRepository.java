package com.desafio.lotus.task.repository;
...
public interface TaskRepository extends JpaRepository<Task, UUID>, JpaSpecificationExecutor<Task> {
    ...
    List<Task> findTop5ByProjectUserIdAndDueDateIsNotNullAndStatusNotOrderByDueDateAsc(
            UUID userId,
            com.desafio.lotus.task.model.TaskStatus status
    );

    long countByProjectId(UUID projectId);

    long countByProjectIdAndStatus(UUID projectId, com.desafio.lotus.task.model.TaskStatus status);
